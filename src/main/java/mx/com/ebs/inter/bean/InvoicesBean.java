 ...
 */
public class InvoicesBean extends AbstractBean<Invoice> implements Serializable {
    ...
    private void createModel(){
        model = new LazyDataModel<Invoice>() {
            @Override
            public List<Invoice> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, Object> filters) {
                ...
                model.setRowCount(invoiceService.countRowsUsingFilter(recInvoiceSearchBo));
                return invoiceService.getListUsingFilter(recInvoiceSearchBo, first, pageSize,sortField,sortOrder);
            }
        };
    }
